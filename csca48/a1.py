...
class Matrix():
    ...
    def __init__(self, m, n, default=0):
        ...
        cur_row = self._head
        for i in range(m):
            cur_row.set_right(MatrixNode(i))
            cur_row = cur_row.get_right()
        
        cur_col = self._head
        for j in range(n):
            cur_col.set_down(MatrixNode(j))
            cur_col = cur_col.get_down()
    ...
    def set_val(self, i, j, new_val):
        ...
        '''
        matrix_node = MatrixNode(new_val)
        cur_row = self._head
        for m in range(i):
            cur_row = cur_row.get_right()
        cur_row.set_down(matrix_node)
